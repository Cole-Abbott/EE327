...
void imu_loop_task(void *parameter)
{
    ...
    while (1)
    {
        ...
        // read the gyro data
        uint8_t gyroDataAddr = 0x43;
        uint8_t gyroDataRead[6];
        I2CWrite(&bbi2c, MPU, &gyroDataAddr, 1);
        I2CRead(&bbi2c, MPU, gyroDataRead, 6);
        int16_t z = (gyroDataRead[4] << 8) | gyroDataRead[5];
